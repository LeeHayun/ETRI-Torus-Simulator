...
#define PAYLOAD_SZ (4096)*1024 /* size of simulated data payload, bytes  */
#define FS_OVERHEAD 10000000//(1000000000.0)
//#define DEBUG_FUNC

#define READ    0
#define WRITE   1

#define FILENAME "obj_trace.txt"
//#define OPTION1
//#define OPTION2
...
static int trace_init()
{
	...
	if (trace_fp == NULL)
	{
		printf("trace file is not exist.\n");
		return -1;
	}
...
static int trace_get()
{
	...
	fgets(buf, BUF_SIZE, trace_fp);
	if (sscanf(buf, "%d %d", &ope, &addr) == EOF)
	{
		return -1;
	}
...
#ifdef OPTION2
    if (addr >= 2000) {
        int fastnode[6] = {1,5,6,30,36,180};
        return fastnode[addr%6]*2;
    }
#endif

	return (addr%(num_servers-1) + 1)*2;
}
...
switch(net_id){
    case SIMPLENET: printf("SIMPLENET\n"); break;
    case SIMPLEP2P: printf("SIMPLEP2P\n"); break;
    case TORUS: printf("TORUS\n"); break;
    case DRAGONFLY: printf("DRAGONFLY\n"); break;
    case LOGGP: printf("LOGGP\n"); break;
    default: break;
}
...
printf("[%lu] %s funcion start\n", lp->gid/2, __func__);
    ...
    //e = codes_event_new(lp->gid, kickoff_time, lp);	// Old version?
    e = tw_event_new(lp->gid, kickoff_time, lp);
    m = tw_event_data(e);
    m->svr_event_type = KICKOFF;
    tw_event_send(e);
#ifdef DEBUG_FUNC
printf("[%lu] %s funcion end\n", lp->gid/2, __func__);
#endif
    return;
}
...
printf("[%lu] %s funcion start\n", lp->gid/2, __func__);
    ...
    printf("server %llu sent %d Mbytes in %f seconds, %f MiB/s sent_count %d recvd_count %d local_count %d \n", (unsigned long long)lp->gid/2, (PAYLOAD_SZ/1024/1024)*ns->msg_sent_count, ns_to_s(ns->end_ts-ns->start_ts), 
        (((double)(PAYLOAD_SZ/1024/1024)*ns->msg_sent_count)/ns_to_s(ns->end_ts-ns->start_ts)), ns->msg_sent_count, ns->msg_recvd_count, ns->local_recvd_count);
#ifdef DEBUG_FUNC
printf("[%lu] %s funcion end\n", lp->gid/2, __func__);
#endif
    return;
...
//printf("dragonfly routers %d\n", num_routers_per_rep);
    ...
    if (do_pull){
        ...
        dest = trace_get();
	    if (dest != -1) {
		    model_net_event(net_id, "test", dest, PAYLOAD_SZ, 0.0,
		     sizeof(svr_msg), (const void*)m_remote, sizeof(svr_msg), (const void*)m_local, lp);
		ns->msg_sent_count++;
    	}
#else
        while ((dest=trace_get()) != -1) {
		    model_net_event(net_id, "test", dest, PAYLOAD_SZ, 0.0,
		     sizeof(svr_msg), (const void*)m_remote, sizeof(svr_msg), (const void*)m_local, lp);
		    ns->msg_sent_count++;
	    }
...
//    m_local->svr_event_type = REQ;
 ...
 //   printf("handle_ack_event(), m->src %d lp->gid %d \n", m->src, lp->gid);
    int opt_offset = 0;
   ...
   if(net_id == DRAGONFLY && (lp->gid % lps_per_rep == num_servers_per_rep - 1))
    ...
    ns->end_ts = tw_now(lp)+FS_OVERHEAD;
...
//    assert(lp->gid == (m->src + offset + opt_offset)%(num_servers*2 + num_routers));
   ...
   // mm Q: What should be the size of an ack message? may be a few bytes? or larger..? 
    model_net_event(net_id, "test", m->src, PAYLOAD_SZ, FS_OVERHEAD, sizeof(svr_msg), (const void*)m_remote, sizeof(svr_msg), (const void*)m_local, lp);
	//printf("%lu\n", m->src);
